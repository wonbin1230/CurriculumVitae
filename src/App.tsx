...
const App = () => {
	...
	const handleTheme = ($isDarkMode: boolean) => {
		if ($isDarkMode) {
			...
			document.documentElement.style.setProperty("--object-color", "linear-gradient(45deg, #D6ECF0, #5F5F5F)");
		}
        else {
			setIsDarkMode(false);
	...
	return (
		<>
			<BrowserRouter>
				<Container>
					<Routes>
						<Route path="/" element={<Nav $isDarkMode={isDarkMode} $handleTheme={handleTheme} />}>
